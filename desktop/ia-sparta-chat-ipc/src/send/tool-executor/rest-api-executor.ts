...
import { googleApiBaseUrls, resolveField } from './types'
import { markdownToNotionBlocks } from './notion-helpers'

/** Mapeo de herramientas MCP a endpoints REST de Google, Microsoft Graph y Notion */
export function resolveGoogleApiCall(
  serverId: string,
  toolName: string,
  input: Record<string, unknown>
): { url: string; method: string; body?: unknown; contentType?: string; rawBody?: string } | null {
...
/** Ejecuta una llamada directa a la API REST de Google/MS/Notion con el token OAuth */
export async function executeGoogleApiCall(serverId: string, toolName: string, input: Record<string, unknown>): Promise<string> {
