...
def _validate_url(url: str) -> str:
    """Validate a URL/command, raising HTTPException on failure.

    Delegates to the shared ``validate_url`` in ``mcp_server_actions`` and
    translates error strings into HTTP 400 responses.
    """
    validated, error = _validate_url_safe(url)
    if error:
