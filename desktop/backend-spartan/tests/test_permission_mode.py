...
# ── MCP server CRUD risk classification ─────────────────────────────


def test_list_mcp_servers_is_never_high_risk():
    assert is_high_risk_tool_call("list_mcp_servers", {}) is False


def test_test_mcp_server_is_never_high_risk():
    assert is_high_risk_tool_call("test_mcp_server", {"address": "https://x.com/mcp"}) is False


@pytest.mark.parametrize("tool", ["add_mcp_server", "update_mcp_server", "delete_mcp_server"])
def test_mcp_crud_mutating_tools_are_high_risk(tool):
    assert is_high_risk_tool_call(tool, {"display_name": "X", "address": "https://x.com"}) is True


def test_list_mcp_servers_is_never_potentially_unsafe():
    assert is_potentially_unsafe_tool_call("list_mcp_servers", {}) is False


def test_test_mcp_server_is_never_potentially_unsafe():
    assert is_potentially_unsafe_tool_call("test_mcp_server", {"address": "https://x.com/mcp"}) is False


@pytest.mark.parametrize("tool", ["add_mcp_server", "update_mcp_server", "delete_mcp_server"])
def test_mcp_crud_mutating_tools_are_potentially_unsafe(tool):
    assert is_potentially_unsafe_tool_call(tool, {"display_name": "X"}) is True
