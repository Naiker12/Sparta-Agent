...
"""Which side runs the code when a turn mixes the Code pill with a Studio tool.

``code_execution`` runs in the provider's sandbox; ``python`` / ``terminal`` run
on the machine Studio is installed on. They are two trust boundaries, not two
spellings of one feature, so the request says which one it wants by name and
this server forwards accordingly. Studio has no implementation of
...
The one case that IS a substitution is a request naming both, which no Studio
...
def test_web_search_is_still_never_forwarded():
    """Studio's catalog does contain web_search, so that one really is replaced."""
