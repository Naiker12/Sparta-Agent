...
# ── mcp_server_actions service layer ────────────────────────────────

import json as _json
from core.inference import mcp_server_actions


def test_list_servers_for_model_returns_json(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    mcp_servers_db.create_server(id = "srv1", display_name = "GitHub", url = "https://github.com/mcp")
    mcp_servers_db.create_server(id = "srv2", display_name = "GitLab", url = "https://gitlab.com/mcp")
    result = mcp_server_actions.list_servers_for_model()
    data = _json.loads(result)
    assert isinstance(data, list)
    assert len(data) == 2
    names = {s["display_name"] for s in data}
    assert names == {"GitHub", "GitLab"}
    # Must not expose headers_json
    for s in data:
        assert "headers_json" not in s
        assert "headers" not in s


def test_list_servers_for_model_empty(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    result = mcp_server_actions.list_servers_for_model()
    assert "No MCP servers" in result


def test_add_server_for_model_validates_url(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    result = mcp_server_actions.add_server_for_model({
        "display_name": "Bad",
        "address": "",
    })
...
def test_add_server_for_model_validates_display_name(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    result = mcp_server_actions.add_server_for_model({
        "display_name": "",
...
def test_add_server_for_model_deduplicates(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    mcp_servers_db.create_server(id = "existing", display_name = "QGIS", url = "https://qgis.com/mcp")
    result = mcp_server_actions.add_server_for_model({
        "display_name": "QGIS",
        "address": "https://qgis.com/mcp",
    })
    assert "already exists" in result


def test_delete_server_for_model_nonexistent(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    result = mcp_server_actions.delete_server_for_model({"server_id": "bogus123"})
    assert "not found" in result.lower()


def test_delete_server_for_model_missing_id():
    result = mcp_server_actions.delete_server_for_model({})
    assert "server_id" in result.lower()


def test_update_server_for_model_nonexistent(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    result = mcp_server_actions.update_server_for_model({
        "server_id": "bogus123",
...
def test_update_server_for_model_no_changes(tmp_path, monkeypatch):
    _reset_db(tmp_path, monkeypatch)
    mcp_servers_db.create_server(id = "srv1", display_name = "A", url = "https://a/m")
    result = mcp_server_actions.update_server_for_model({"server_id": "srv1"})
