...
# Canonical CUDA spoof at the repo root (studio/spartan_backend -> studio -> repo root). Loaded by the
...
# Runs in a fresh interpreter with cwd == studio/spartan_backend so ``utils.*`` resolves like the worker.
