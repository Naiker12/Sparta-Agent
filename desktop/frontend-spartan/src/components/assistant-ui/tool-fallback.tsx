...
function ToolFallbackResult({
  ...
  // Special-case: list_mcp_servers returns a JSON array of server objects.
  if (toolName === "list_mcp_servers") {
    const listContent = renderMcpServerList(result);
    if (listContent) {
      return (
        <div
  ...
  return (
    ...
    val.every(
      (entry: unknown) =>
        typeof entry === "object" &&
        entry !== null &&
        typeof (entry as McpServerEntry).display_name === "string" &&
        typeof (entry as McpServerEntry).url === "string",
    )
  );
}

function renderMcpServerList(result: unknown): ReactNode | null {
  // The result can be a JSON string or already parsed
  let parsed = result;
  if (typeof result === "string") {
    // "No MCP servers configured." or similar non-JSON
    try {
