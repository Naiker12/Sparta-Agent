...
export function ElectronUpdateBanner({
  enabled = true,
}: ElectronUpdateBannerProps): ReactElement | null {
  const { available, state, check, download, installAndRestart } =
    useElectronUpdate();
  const t = useT();
  ...
  return (
    <div
      className="pointer-events-auto flex w-[calc(100vw-2rem)] max-w-[448px] flex-col rounded-[20px] border border-border/70 bg-card p-5 shadow-2xl backdrop-blur-xl"
    ...
    >
      ...
      {state.version && (
        <ReleaseNotesPanel
          ...
          open={showNotes}
          fallbackMarkdown={state.releaseNotes}
        />
      )}
      {state.stage === "available" && (
        <div className="mt-4 flex items-center justify-between gap-2 border-t border-border/40 pt-3">
          ...
          <Button size="sm" className="gap-1.5" onClick={() => void download()}>
