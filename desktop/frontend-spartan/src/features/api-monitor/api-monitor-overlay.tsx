      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            onPointerDown={() =>
              useFloatingPanelOrderStore.getState().raise("api-monitor")
            }
            onDragEnd={() => {
              // The panel may have settled slightly outside the viewport before drag;
              // this first user drag makes sure it is fully on screen.
            ...
            onAnimationComplete={() => setSettledAt(performance.now())}
            className="unsloth-glass-card pointer-events-auto absolute flex w-[320px] flex-col rounded-[20px] p-2.5 shadow-2xl"
          >
            <div className="flex items-center justify-between gap-2 px-1.5 pb-2 pt-0.5">
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                ...
                <button
                  type="button"
                  onClick={close}
                  title={t("apiPage.overlay.closeTooltip")}
                  aria-label={t("apiPage.overlay.closeAria")}
            ...
            <p className="truncate px-1.5 pb-2.5 text-ui-11p5 tracking-nav text-muted-foreground">
              {data?.active_model ?? t("apiPage.overlay.noModelLoaded")}
            </p>
            ...
            <div className="flex flex-col pb-1 pt-1.5">
              {entries.length === 0 ? (
                <p className="px-1.5 py-4 text-center text-ui-11p5 tracking-nav text-muted-foreground">
                  {t("apiPage.overlay.noRequestsYet")}
                </p>
              ) : (
            ...
            >
              ...
              />
              {t("apiPage.overlay.expandToFull")}
            </button>
            ...
            >
              {t("apiPage.overlay.stopAutoOpen")}
            </button>
          </motion.div>
        </div>
      )}
