...
function PayloadBlock({
  ...
  return (
    <section className="flex min-w-0 flex-col gap-1.5">
      ...
      >
        {loading && !body ? t("apiPage.loading") : body || "–"}
      </pre>
    </section>
  );
}
    ...
    <div className="flex min-w-0 flex-col gap-5 p-5">
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-3 rounded-xl border border-border/60 bg-card px-4 py-3 sm:grid-cols-3">
        {[
          ...
          {
            label: t("apiPage.completionTokens"),
            value:
          ...
          {
            label: t("apiPage.generationSpeed"),
            value: formatTokPerSec(entry.tok_per_sec) ?? "–",
          },
          {
            label: t("apiPage.stopReason"),
      ...
      <PayloadBlock
        title={t("apiPage.promptSection")}
        body={prompt}
        truncated={entry.prompt_truncated && detail?.prompt == null}
        loading={loading}
      />
      <PayloadBlock
        title={t("apiPage.replySection")}
        body={reply}
...
export function ApiMonitorPage(): ReactElement {
  ...
  const statusCopy =
    serverStatus === "generating"
      ? t("apiPage.statusServing")
      : serverStatus === "ready"
        ? t("apiPage.statusReady")
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 pb-10 pt-12 font-heading sm:px-10">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <HugeiconsIcon
              ...
              className="size-4"
            />
            {paused ? t("apiPage.resume") : t("apiPage.pause")}
          </Button>
          <Button
            type="button"
            variant="outline"
            ...
            title={
              data?.active_model
                ? t("apiPage.unloadTitle", { model: data.active_model })
                : t("apiPage.noModelLoadedTooltip")
            }
            className="h-9 gap-1.5 rounded-full"
          >
            <HugeiconsIcon
              ...
              className="size-4"
            />
            {unloading ? t("apiPage.unloading") : t("apiPage.unload")}
          </Button>
          <Button
            type="button"
            variant="outline"
      ...
      <section className="flex flex-wrap items-center gap-x-6 gap-y-3 rounded-xl border border-border/60 bg-card px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <CopyButton value={baseUrl} label={t("apiPage.copyBaseUrl")} />
        </div>
        <div className="flex min-w-0 flex-col">
          <span className="text-ui-10 font-medium uppercase tracking-wider text-muted-foreground">
            {t("apiPage.status")}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <span className="text-ui-10 font-medium uppercase tracking-wider text-muted-foreground">
            {t("apiPage.loadedModel")}
          </span>
          <span className="truncate text-ui-12 text-foreground">
            {data?.active_model ?? t("apiPage.none")}
            {data?.context_length
      ...
      <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-border/60 bg-card">
        <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-4 py-3">
          <Input
            ...
            aria-label={t("apiPage.searchAria")}
          ...
          >
            <SelectTrigger
              aria-label={t("apiPage.filterAria")}
          ...
          <div className="max-h-[560px] min-h-[220px] overflow-y-auto border-b border-border/60 lg:border-b-0 lg:border-r">
            {loading ? (
              ...
              <p className="px-4 py-10 text-center text-sm text-muted-foreground">
                {entries.length > 0
                  ? t("apiPage.noRequestsMatchFilter")
                  : loggingDisabled
                    ? t("apiPage.recordingOffMessage")
                    : t("apiPage.noTrafficYet")}
              </p>
            ) : (
              visible.map((entry) => (
                <RequestRow
