...
for (const catalog of [IMAGE_CATALOG, VIDEO_CATALOG, AUDIO_CATALOG]) {
  for (const group of catalog) {
    for (const artifact of group.artifacts) {
      if (!artifact.totalParams || artifact.format === "gguf") {
        continue;
      }
      assert.ok(
...
assert.equal(
  groupForRepoId("QuantStack/HunyuanImage-2.1-GGUF", IMAGE_CATALOG),
  hyimage,
);
  ...
  for (const group of groups) {
    for (const artifact of group.artifacts) {
      ...
      if (bucket) {
        ...
        artifactsByRepo.set(artifact.repoId, [artifact]);
      }
    }
  }
  ...
  await inBatches(
    ...
    async ([repoId, artifacts]) => {
      ...
      for (const filename of declaredFiles) {
        ...
        if (hubGated) {
          continue; // resolve/ 401s without a token; the sibling list is the check.
        }
