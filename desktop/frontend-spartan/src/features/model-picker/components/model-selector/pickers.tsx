...
} from "./fine-tuned-rows";
import { FolderBrowser } from "./folder-browser";
import {
  GgufVariantExpander,
  type SoleDownloadedQuant,
  ggufVariantExpectedBytes,
  normalizeGgufVariantsResponse,
  useSoleDownloadedQuants,
} from "./gguf-variant-expander";
import { curatedArtifactIsOfferable } from "./host-artifact-policy";
import { missingExternalModel } from "./missing-external-model";
import {
  CapabilityScope,
  ListLabel,
  SizeText,
...
import { isUnslothOwner, splitRepoLabel } from "./row-meta";
import type {
  DeletedModelRef,
  ExternalModelOption,
  LoraModelOption,
...
function mediaPageForTask(
  ...
  if (!(task && MEDIA_PAGE_TASKS.includes(task))) {
    return null;
  }
  if ((VIDEO_GEN_TASKS as readonly string[]).includes(task)) {
    return "video";
  }
  if ((AUDIO_GEN_TASKS as readonly string[]).includes(task)) {
    return "audio";
  }
  return "images";
}
...
function isImageEditModel(repoId: string | null | undefined): boolean {
  ...
  if (SUPPORTED_EDIT_KEYWORDS.some((kw) => idHasSegment(id, kw))) {
    return false;
  }
...
const _FORMAT_FILTER_OPTIONS: HubOption<FormatFilter>[] = (
...
export function HubModelPicker({
  ...
  const isExt = isExternalModelId(selectedCheckpoint);
  const isExtMissing =
    isExt &&
    Boolean(missingExternalModel(selectedCheckpoint, externalModels ?? [], []));
  const chatLoadedModelId = chatModelLoaded({
    checkpoint: selectedCheckpoint,
    isExternalModel: isExt,
    isExternalMissing: isExtMissing,
    residentCheckpoint,
  })
    ? selectedCheckpoint
    : undefined;
  ...
  >(() => {
    ...
    if (VIDEO_GEN_TASKS.some((t) => tasks.includes(t))) {
      return ["audio"];
    }
    if (IMAGE_GEN_TASKS.some((t) => tasks.includes(t))) {
      return [];
    }
    if (AUDIO_GEN_TASKS.some((t) => tasks.includes(t))) {
      return [];
    }
    return null;
  }, [task]);
  ...
  const curatedOfferable = useCallback(
    (id: string) => {
      ...
      if (downloadedSet.has(id.toLowerCase())) {
        return true;
      }
  ...
  const pipelineTagById = useMemo(() => {
    ...
    for (const r of [
      ...
      if (r.pipelineTag && !map.has(r.id)) {
        map.set(r.id, r.pipelineTag);
      }
    }
    return map;
  }, [
  ...
  const diffusionTaskById = useMemo(() => {
    ...
    const put = (
      ...
      if (id && task) {
        byId.set(id.toLowerCase(), task);
      }
    };
    for (const c of cachedGguf) {
      put(c.repo_id, c.task);
    }
    for (const c of cachedModels) {
      put(c.repo_id, c.task);
    }
    ...
    for (const m of localDirModels) {
      putLocal(m);
    }
    for (const m of customFolderModels) {
      putLocal(m);
    }
    return byId;
  }, [
  ...
  const visibleAdditionalOnDeviceModels = useMemo(() => {
    ...
    const filtered = additionalOnDeviceModels.filter((model) => {
      ...
      if (!id || alreadyListed.has(id) || seen.has(id)) {
        return false;
      }
      seen.add(id);
      return (
  ...
  const fetchSearchMore = useCallback((): boolean | undefined => {
    ...
    const communityRequested =
      communityDiscoveryEnabled && communityQuerySearch.hasMore
        ...
        : false;
    if (unslothRequested || communityRequested) {
      return true;
    }
    return undefined;
  }, [
  ...
  useEffect(() => {
    if (!(recommendedSentinel && recommendedHasMore)) {
      return;
    }
  ...
  const recommendedSortOptions: HubOption<RecommendedSortKey>[] = useMemo(
    () => [
      { value: "recommended", label: t("picker.recommended") },
      { value: "trendingScore", label: t("picker.trending") },
      { value: "lastModified", label: t("picker.recent") },
    ],
    [t],
  );

  const localSortOptions: HubOption<LocalSortKey>[] = useMemo(
    () => [
      { value: "recent", label: t("picker.recent") },
      { value: "size", label: t("picker.size") },
      { value: "name", label: t("picker.name") },
      { value: "downloaded", label: t("picker.downloaded") },
    ],
    [t],
  );

  const formatFilterOptions: HubOption<FormatFilter>[] = useMemo(
    () => [
  ...
  const sectionSortDropdown =
    section === "recommended" ? (
      ...
      <HubOptionMenu
        ...
        contentClassName={sortMenuContentClassName}
        triggerContent={sortTriggerContent(
          localSortOptions.find((o) => o.value === downloadedSort)?.label ??
            downloadedSort,
        )}
      />
    ) : (
      <HubOptionMenu
        ...
        contentClassName={sortMenuContentClassName}
        triggerContent={sortTriggerContent(
          localSortOptions.find((o) => o.value === customSort)?.label ??
            customSort,
        )}
      />
    );
  ...
  const renderDownloadedGgufRow = (c: (typeof visibleCachedGguf)[number]) => {
    ...
    if (soleQuant) {
      return renderSoleQuantGgufRow(c, soleQuant);
    }
        ...
        >
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={
                section === "downloaded"
                  ? t("picker.searchLocal")
                  : t("picker.searchSpartan")
              }
          ...
          {onBrowseHub ? (
            <Tooltip>
              ...
              <TooltipContent>{t("picker.searchAll")}</TooltipContent>
            </Tooltip>
          ) : null}
        </div>
              ...
              connectedGroups.length === 0 ? (
                <div className="px-2.5 py-2 text-xs leading-relaxed text-muted-foreground">
                  {externalModels.length === 0
                    ? t("picker.noConnectedModels")
                    : t("picker.noMatchingModels")}
                </div>
              ) : (
              ...
              when custom folders below still have matches. */}
                ...
                sortedCustomFolderModels.length === 0 ? (
                  <div className="px-2.5 py-2 text-xs text-muted-foreground">
                    {showHfSection
                      ? t("picker.noMatchingModelsOnDevice")
                      : formatFilter === "all"
                        ? t("picker.noDownloadedModels")
              ...
              Fine-tuned. Shown only when such models exist. */}
                ...
                {showCustom ? (
                  <>
                    ...
                    sortedCustomFolderModels.length === 0 ? (
                      <div className="px-2.5 py-2 text-xs text-muted-foreground">
                        {t("picker.noMatchingCustomFolderModels")}
                      </div>
                    ) : null}
                  </>
                ) : null}
