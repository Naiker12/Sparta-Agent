...
export function buildAgentCommand(
  ...
  const bare = `spartan start ${agent}`;
  ...
  // loopback on the same port must keep its explicit SPARTAN_STUDIO_URL.
  if (
    url.protocol === "http:" &&
    isDefaultLocalHost(host) &&
    url.port === DEFAULT_STUDIO_PORT
  ) {
    return bare;
  }
  ...
  return os === "windows"
    ? `$env:SPARTAN_STUDIO_URL="${studioUrl}"; ${cmd}`
    : `SPARTAN_STUDIO_URL=${studioUrl} ${cmd}`;
}
