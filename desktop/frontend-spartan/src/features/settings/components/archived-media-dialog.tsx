...
export type ArchivedMediaKind = "images";

// Video archival is no longer exposed. These inert compatibility helpers keep
// the shared gallery component safe for persisted pre-removal state while
// ensuring it never calls a video endpoint or displays a video shelf.
type RemovedVideoRow = {
  id: string;
  prompt: string;
  created_at: string;
  url: string;
};
const getVideoGallery = async (..._args: unknown[]) => ({
  videos: [] as RemovedVideoRow[],
  has_more: false,
});
const fetchGalleryVideoSignedUrl = async (..._args: unknown[]) => "";
const setGalleryVideoFlags = async (..._args: unknown[]) => undefined;
const deleteGalleryVideo = async (..._args: unknown[]) => undefined;
