...
const FINE_TUNE_ACTIONS: FineTuneAction[] = ["export", "recipes"];
...
export function DataTab() {
  ...
  const fineTuneAction = FINE_TUNE_ACTIONS.includes(storedFineTuneAction)
  ...
  const handleOpenInRecipes = async () => {
    ...
    try {
      ...
      if (recipeId) {
        navigate({ to: "/data-recipes/$recipeId", params: { recipeId } });
      }
    } catch (error) {
      toast.error(t("settings.data.fineTuneExportFailed"), {
        description: error instanceof Error ? error.message : undefined,
  ...
  const fineTuneBusy = openingRecipe || fineTuneExporting;
  const runFineTuneAction = () => {
    if (fineTuneAction === "recipes") {
      void handleOpenInRecipes();
    } else {
      void handleFineTuneExport();
    }
  };
