...
export function BackendSetupGate() {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] grid place-items-center bg-[#352d40]/35 p-6"
      role="dialog"
      aria-modal="true"
    ...
    >
      <section className="w-full max-w-xl rounded-2xl bg-[#fdf9f2] p-7 text-[#352d40] shadow-2xl">
        ...
        <h1 id="backend-setup-title" className="mt-2 text-2xl font-semibold">
          Configura Sparta Agent
        </h1>
        <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#625b6e]">
          {message}
        </p>
        {progress && (
          <p className="mt-4 rounded-lg bg-[#f2ebe0] px-3 py-2 text-sm text-[#4c4554]">
            {progress}
          </p>
        )}
        <div className="mt-6 flex justify-end gap-3">
          <button
            type="button"
            className="rounded-lg px-4 py-2 text-sm text-[#625b6e]"
          ...
          <button
            type="button"
            className="rounded-lg bg-[#2997ff] px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-60"
