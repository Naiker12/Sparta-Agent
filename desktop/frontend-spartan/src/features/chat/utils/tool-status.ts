...
/** Which glyph the badge shows: exact match for the nudge, "Running"/"Editing" prefix for sandbox tools, globe otherwise. */
export function toolStatusKind(status: string): ToolStatusKind {
  if (status === NUDGE_TOOL_CALLS_STATUS) {
    return "nudge";
  }
  // edit_file reports "Editing: name", not "Running", and it is as local as the
  // other two. Without it a file edit shows the globe, i.e. the web-search glyph.
