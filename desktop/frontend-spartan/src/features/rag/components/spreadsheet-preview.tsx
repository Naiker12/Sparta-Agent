...
export function LocalSpreadsheetPreview({ blob }: { blob: Blob }) {
  ...
  const [selected, setSelected] = useState(() =>
    String(
      Math.max(
        ...
        previewCache
          .get(blob)
          ?.sheets.findIndex((sheet) => sheet.rows.length > 0) ?? 0,
      ),
    ),
  );
  ...
  useEffect(() => {
    ...
    const worker = new Worker(
      new URL("./spreadsheet-preview.worker.ts", import.meta.url),
      { type: "module" },
    );
    const fail = (message: string) => {
      setResult({ blob, sheets: [], error: message });
      worker.terminate();
    };
    const timer = window.setTimeout(
      () => fail(t("chat.preview.localSafetyError")),
      15000,
    );
    worker.onmessage = ({ data }) => {
      ...
      if (!data.error) {
        previewCache.set(blob, { ...data, sheets });
      }
    ...
    worker.onerror = () => {
      clearTimeout(timer);
      fail(t("chat.preview.unsupportedSpreadsheet"));
    };
  ...
  if (result.error) {
    return (
      <p className="p-6 text-sm text-muted-foreground" role="alert">
        {t("chat.preview.spreadsheetError", { error: result.error })}
      </p>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        >
          <thead className="sticky top-0 z-10 bg-muted">
            <tr>
              <th className="border-b border-r p-2" scope="col">
                #
              </th>
              {Array.from({ length: columns }, (_, i) => (
                <th
                  key={i}
                  scope="col"
                  className="min-w-32 border-b border-r p-2 text-left font-medium"
                >
                  {columnLabel(i)}
                </th>
              ))}
          ...
          <tbody>
            {rows.slice(page * 100, (page + 1) * 100).map((row, i) => (
              <tr key={i} className="hover:bg-muted/50">
                <th
                  scope="row"
                  className="sticky left-0 border-b border-r bg-muted px-3 py-2 font-normal text-muted-foreground"
                >
                  {page * 100 + i + 1}
                </th>
                {Array.from({ length: columns }, (_, j) => (
                  <td
                    key={j}
                    className="max-w-96 whitespace-pre-wrap break-words border-b border-r px-3 py-2 align-top"
                  >
                    {row[j] ?? ""}
                  </td>
                ))}
        ...
        {rows.length === 0 && (
          <p className="p-6 text-sm text-muted-foreground">
            {t("chat.preview.emptySpreadsheet")}
          </p>
        )}
      </div>
      ...
      <Tabs
        ...
        className="overflow-x-auto border-t p-2"
      >
        <TabsList>
          {result.sheets.map((item, i) => (
