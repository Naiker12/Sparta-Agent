  ...
  try {
    ...
    const sheets = names.slice(0, 32).map((name) => {
      ...
      return {
        name,
        rows: rows.map((row) =>
          row.map((cell) => String(cell ?? "").slice(0, 1000)),
        ),
        truncated:
          Boolean(sheet["!fullref"]) ||
          xlsx.utils.decode_range(sheet["!ref"]).e.r > 499 ||
          xlsx.utils.decode_range(sheet["!ref"]).e.c > 63,
      };
    });
...
if (typeof self !== "undefined") {
  self.onmessage = async (event: MessageEvent<Blob>) => {
    self.postMessage(await parseSpreadsheetPreview(event.data));
  };
}
