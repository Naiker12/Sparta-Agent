...
export function PreviewWorkspace() {
  ...
  const source = useDocumentPreviewStore(
    (state) => state.localPreview?.workspaceSource,
  );
  ...
  const refreshGit = useCallback(async () => {
    ...
    const result = await getProjectNativeFilesystem()?.getGitStatus?.(
      project.id,
    );
    setGit(result ?? null);
  }, [project]);
  ...
  const root = project.connectedFolderPath
    .replace(/\\/g, "/")
    .replace(/\/$/, "");
  ...
  const relative =
    ...
    path?.toLowerCase().startsWith(`${root.toLowerCase()}/`)
      ...
      : null;
  return (
    <div className="flex min-w-0 flex-col border-b">
      <div className="flex min-w-0 items-center gap-1 px-3 py-1">
        <WorkspaceFolderChip />
        {relative && (
          <nav
            aria-label={t("projectsPage.browseFolder")}
            title={relative}
            className="flex min-w-0 flex-1 items-center overflow-hidden text-xs text-muted-foreground"
          >
            {relative.split("/").map((part, index, parts) => (
      ...
      {git?.isRepository && (
        <div className="flex min-w-0 items-center gap-2 border-t px-3 py-1 text-xs text-muted-foreground">
          ...
          {git.changed ? (
            <span className="shrink-0">
              {t("projectsPage.gitChanges")} +{git.insertions ?? 0} −
              {git.deletions ?? 0}
            </span>
          ) : (
            <span className="shrink-0">{t("projectsPage.gitNoChanges")}</span>
          )}
          {!!(git.ahead || git.behind) && (
            <span className="truncate">
              {t("projectsPage.gitAheadBehind", {
                ahead: git.ahead ?? 0,
                behind: git.behind ?? 0,
              })}
            </span>
      ...
      {git && !git.isRepository && (
        <p className="border-t px-3 py-1 text-xs text-muted-foreground">
          {t("projectsPage.gitNoRepository")}
        </p>
      )}
