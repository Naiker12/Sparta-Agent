export const agents = {
  title: "Agents",
  description:
    "Connect coding agents like Claude Code and Codex to a local model with Spartan start.",
  intro:
    "connects Claude Code, Codex, Hermes, OpenClaw, OpenCode and other agents to a model served locally by Spartan, fully offline. It runs an OpenAI-compatible server and never touches your agent's config files.",
  ...
  modelNote:
    "Codex requires a GGUF model served by llama-server. Other agents can also use transformer-backed models; remove --model to use the model already loaded in Spartan.",
  subagent: {
    title: "Use a local model as a subagent",
    description:
      "Keep {agent} on its current model and delegate selected tasks to this local Spartan model.",
  ...
  options: {
    ...
    description:
      "Spartan flags are parsed first; anything it doesn't recognize is passed straight through to the agent.",
    model:
      "Select a model. Without --model, Spartan start uses the model currently loaded in Studio and errors if none is loaded.",
    contextLength:
    ...
    asSubagent:
      "Keep the parent on its current model and register Spartan as a local subagent (Claude Code, Codex, and OpenCode).",
  ...
  remote: {
    ...
    description:
      "Point Spartan start at a Studio running elsewhere by setting these before launching (or pass --api-key directly):",
  },
  passthrough: {
    title: "Passing agent arguments",
    description:
      "Arguments after the Spartan flags are forwarded to the agent itself, so native commands like resume still work:",
  },
  dryRun: {
