...
function matchLocale(tag: string): Locale | null {
  ...
  if (language === "zh" && isTraditionalChinese(lower)) {
    return null;
  }
  return (
...
function detectLocale(): Locale {
  ...
  const tags =
    navigatorRef?.languages && navigatorRef.languages.length > 0
...
function failPreference(
  ...
  if (revision !== preferenceRevision || pendingPreference === null) {
    return;
  }
  pendingPreference = null;
  pendingPreferenceShouldPersist = false;
...
function isLocaleStorageEvent(event: StorageEvent): boolean {
  ...
  if (!event.storageArea || typeof window === "undefined") {
    return true;
  }
...
function handleLanguageChange(): void {
  ...
  if (effectivePreference !== AUTO_LOCALE) {
    return;
  }
...
function startListeners(): void {
  if (areListenersActive || typeof window === "undefined") {
    return;
  }
