...
//
//   1. "@tauri-apps/api/core" -> a local stub, because the real package only resolves
//      inside a Tauri webview.
//   2. A "?bust=N" query on the importer is copied onto every src module it pulls in.
//      lib/api-base computes `isTauri` once, at module evaluation, so the only way to
//      exercise both branches in one process is to evaluate the subgraph again.
import { existsSync } from "node:fs";
import { fileURLToPath, pathToFileURL } from "node:url";

const SRC = fileURLToPath(new URL("../../src/", import.meta.url));
const CORE_STUB = new URL("./tauri-core-stub.mjs", import.meta.url).href;
...
export function resolve(specifier, context, next) {
  ...
  if (specifier === "@tauri-apps/api/core") {
    return next(CORE_STUB + suffix, context);
  }
