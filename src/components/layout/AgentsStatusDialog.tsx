  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Agentes en vivo</DialogTitle>
        </DialogHeader>
        <div style={{ padding: '0 24px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {runningAgents.length === 0 ? (
            ...
            runningAgents.map((agent) => {
              ...
              return (
                <div key={agent.id} style={{ padding: '12px 14px', borderRadius: 'var(--radius-lg)', background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', cursor: onFocusAgent ? 'pointer' : 'default' }}
                  onClick={() => { if (onFocusAgent) { onFocusAgent(agent.id); onClose() } }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: agent.status === 'thinking' ? 'var(--status-think)' : 'var(--status-warn)', flexShrink: 0 }} />
                    <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', fontFamily: 'var(--font-ui)', flex: 1 }}>{agent.name}</span>
                    <span style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'var(--font-ui)' }}>{agent.status === 'thinking' ? 'Pensando' : 'Ejecutando'}</span>
                    {onFocusAgent && <ChevronRight size={14} style={{ color: 'var(--text-muted)' }} />}
                  </div>
                  {latestTask && (
                    <div style={{ fontSize: 11, color: 'var(--text-secondary)', fontFamily: 'var(--font-ui)', marginBottom: 6, lineHeight: 1.4 }}>
