...
export function GatewayStatusDialog({ open, onClose }: GatewayStatusDialogProps) {
  ...
  function handleConfigure(_providerId: string) {
    onClose()
    openSettings()
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Gateway de API</DialogTitle>
        </DialogHeader>
        <div style={{ padding: '0 24px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 'var(--radius-md)', background: hasRecentError ? 'rgba(239,68,68,0.08)' : 'rgba(34,197,94,0.08)' }}>
            {hasRecentError ? (
              <AlertCircle size={16} style={{ color: 'var(--status-err)', flexShrink: 0 }} />
            ) : (
              <CheckCircle size={16} style={{ color: 'var(--status-ok)', flexShrink: 0 }} />
            )}
            <span style={{ fontSize: 12, color: hasRecentError ? 'var(--status-err)' : 'var(--status-ok)', fontFamily: 'var(--font-ui)', fontWeight: 500 }}>
          ...
          {providers.length === 0 ? (
            <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontFamily: 'var(--font-ui)', textAlign: 'center', padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {providers.map((p) => {
                ...
                return (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 'var(--radius-md)', background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)' }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: isHealthy ? 'var(--status-ok)' : 'var(--status-err)', flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-primary)', fontFamily: 'var(--font-ui)' }}>{p.label}</div>
                    ...
                    {lastEntry && !lastEntry.ok && (
                      <div style={{ fontSize: 10, color: 'var(--status-err)', fontFamily: 'var(--font-ui)', textAlign: 'right', maxWidth: 140 }}>
                        {lastEntry.error ?? `HTTP ${lastEntry.status}`}
                      </div>
                    )}
                    <button
                      onClick={() => handleConfigure(p.id)}
                      style={{ fontSize: 10, padding: '3px 8px', background: 'var(--bg-hover)', border: '1px solid var(--border-normal)', borderRadius: 'var(--radius-sm)', color: 'var(--text-secondary)', cursor: 'pointer', fontFamily: 'var(--font-ui)', whiteSpace: 'nowrap' }}
                    >
                      Configurar
                    </button>
                  </div>
          ...
          {errorEntries.length > 0 && (
            <>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--status-err)', fontFamily: 'var(--font-ui)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Últimos errores ({errorEntries.length})
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 120, overflowY: 'auto' }}>
                {errorEntries.slice(0, 5).map((entry, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-input)' }}>
