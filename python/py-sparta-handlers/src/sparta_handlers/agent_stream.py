...
from sparta_handlers.agent_prepare import prepare_agent
...
async def run_agent_stream(
    *,
    ...
    from sparta_tools.permission_broker import set_agent_autonomy as _set_autonomy, set_current_session as _set_session

    _set_session(session_id)
    _set_autonomy(agent_autonomy)

    if open_files is not None:
        from sparta_tools.terminal_tools import _set_open_files
        _set_open_files(list(open_files))

    if agent_execute_local is not None:
        from sparta_tools.terminal_tools import set_execute_local, set_sandbox_mode
    ...
    if workspace_root:
        ...
        from sparta_hooks.events import SESSION_STOP
