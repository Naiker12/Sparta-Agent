...
from sparta_config.providers import OpenAICompatibleTransport, build_llm
from sparta_config.providers.cache import clear_llm_cache
from sparta_config.providers.registry import _get_transport

pytest.register_assert_rewrite("sparta_providers.tests.test_providers")
...
def test_build_llm_sets_correct_temperature_and_max_tokens(
    ...
    with patch("sparta_config.providers.factory._get_transport", return_value=mock_transport), \
         patch("sparta_config.providers.factory._llm_cache_get", return_value=None):
        build_llm(
            model="test-model",
            provider=vendor,
