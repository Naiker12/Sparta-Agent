...
# Detect JSON blocks that look like internal planning ({"needs_planning":..., "steps":...})
# These are generated by the planner node but sometimes leak into the agent response.
_JSON_PLAN_RE = re.compile(r'\{\s*"needs_planning"\s*:\s*(true|false)\s*,\s*"steps"\s*:\s*\[', re.IGNORECASE)
_JSON_PLAN_CLOSE_RE = re.compile(r'\}\s*$')
...
class StreamingThinkScrubber:
    ...
    def feed(self, text: str) -> tuple[str, str]:
        ...
        while self._buf:
            # Detect JSON planning blocks leaked from planner — suppress as reasoning
            json_plan_match = _JSON_PLAN_RE.search(self._buf)
            if json_plan_match and json_plan_match.start() == 0:
                # Find the closing of the JSON block (look for matching structure)
                depth = 1
                end = len(self._buf)
                for i in range(json_plan_match.end(), len(self._buf)):
                    ch = self._buf[i]
                    if ch == '{':
                        depth += 1
                    elif ch == '}':
