  ...
  return (
    <header
      className={`sticky top-0 z-50 h-16 w-full border-b transition-all duration-300 ${
        scrolled
          ? 'bg-white/95 dark:bg-[#09090b]/90 backdrop-blur-xl border-slate-200 dark:border-white/10 shadow-lg shadow-slate-900/5 dark:shadow-black/40'
    ...
    >
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between gap-4 border-x border-slate-200 dark:border-white/10 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <a href={getPublicUrl('docs')}>
            <button className="px-4 py-2 rounded-xl border border-slate-300 dark:border-white/15 bg-white/70 hover:bg-slate-100 text-slate-800 dark:bg-white/[0.04] dark:hover:bg-white/10 dark:text-gray-200 font-mono font-bold text-xs transition-all cursor-pointer flex items-center gap-2">
              <span>Docs</span>
          ...
          >
            <button className="px-4 py-2 rounded-xl bg-[#18181b] hover:bg-[#27272a] text-white font-mono font-bold text-xs shadow-md shadow-[#18181b]/30 transition-all cursor-pointer flex items-center gap-2 border border-[#18181b]">
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="border-b border-slate-200 bg-white px-4 py-6 dark:border-white/10 dark:bg-[#09090b] lg:hidden">
          <nav className="flex flex-col gap-4" aria-label="Navegación móvil">
            ...
            <div className="mt-2 flex gap-3 border-t border-slate-200 pt-4 dark:border-white/10">
              <a href={getPublicUrl('docs')} onClick={() => setMobileMenuOpen(false)} className="inline-flex flex-1 items-center justify-center rounded-xl border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-800 dark:border-white/15 dark:text-gray-200">Docs</a>
              <a href="https://github.com/Naiker12/Sparta-Agent" target="_blank" rel="noopener noreferrer" className="inline-flex flex-1 items-center justify-center rounded-xl bg-[#18181b] px-3 py-2 text-xs font-semibold text-white">GitHub</a>
            </div>
          </nav>
        </div>
      )}
