...
export function Architecture() {
  const layers = [
    {
      ...
      icon: Layout,
      color: 'border-sky-500/40 text-sky-500 bg-sky-500/10',
  ...
  return (
    <section id="arquitectura" className="py-24 md:py-32 relative bg-[var(--bg-base)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <Badge variant="accent" className="px-3.5 py-1 text-xs uppercase font-mono tracking-widest">
          ...
          {layers.map((layer, index) => {
            ...
            return (
              <div key={layer.step} className="relative">
                <Card className="border-[var(--border-normal)] bg-[var(--bg-surface)] p-6 md:p-8 hover:border-[var(--accent)] transition-all duration-300 inset-highlight">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 pb-4 border-b border-[var(--border-subtle)]">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <span className="text-xs font-mono text-indigo-500 dark:text-indigo-400 font-bold">{layer.step}</span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                    {layer.features.map((feat, fIdx) => (
                      <div key={fIdx} className="text-xs font-mono font-medium text-[var(--text-primary)] bg-[var(--bg-base)] p-3 rounded-lg border border-[var(--border-normal)]">
                        • {feat}
                      </div>
                    ))}
                  </div>
                ...
                {index < layers.length - 1 && (
                  <div className="flex justify-center my-4 text-indigo-500">
